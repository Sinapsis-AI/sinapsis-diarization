...
from sinapsis_diarization.pipelines.asr_emotion_diarization import ASRDiarizationEmotionPipeline

NO_MODEL = "NO_MODEL_NAME"


def run_asr_diarization_emotion(
    audio_path: str,
    asr_model: str,
    diarization_model: str,
    emotion_model: str = "speechbrain",
    chunk_size: int = -1,
    ...
    model: ASRDiarizationEmotionPipeline
    ...
    if emotion_model != "speechbrain":
        raise NotImplementedError(f"{emotion_model} is not an implemented model")
    if diarization_model == "pyannote":
        ...
        model = ASRDiarizationEmotionPipeline(
            ...
            emotion_pipeline=emotion_model,
        ...
        raise NotImplementedError(f"{diarization_model_name} is not an implemented model")
    audio_config = AudioConfig(sample_rate)
    transcription = model.transcribed_diarization(audio_path, audio_config)
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    output_file = os.path.join(
        output_dir, f"transcribed_diarization_{asr_model}_{diarization_model}_{emotion_model}.txt"
    )
    ...
    with open(output_file, "w") as output_obj:
        for turn in transcription.segments:
            ...
            for emotion in turn.emotions:
                emotions = emotions + " " + emotion.name
                if emotion.score is not None:
                    emotions = emotions + ": " + str(emotion.score)
            output_obj.write(f"{turn.speaker}: {turn.text}, emotions: {emotions}\n")
